...
full$weight <- apply(1 / (exp(-weight_preds$weight) + 1), 1, mean)


library(tidyverse)
full |>
    filter(time_id == 77) |>
    ggplot(aes(x = x, y = y, color = weight)) +
    geom_tile(size = 4) +
    geom_point(data = cmaq_aqs_matched |>
                   filter(time_id == 76),
               aes(x = x, y = y), 
               color = "black",
               size = 3)
...
unique(modis_full$time_id)
modis_full |>
    filter(time_id == 75) |>
    ggplot(aes(x = x, y = y, color = aod)) +
    geom_tile(size = 4) +
    geom_point(data = cmaq_aqs_matched |>
                   filter(time_id == 77),
               aes(x = x, y = y), 
...
ctm_pred$x <- cmaq_full$x
ctm_pred$y <- cmaq_full$y
ctm_pred$estimate <- ctm_pred$estimate[, 1]

ctm_pred |>
    filter(time.id == 77) |>
    ggplot(aes(x = x, y = y, color = estimate)) +
    geom_tile(size = 4) +
    geom_point(data = cmaq_aqs_matched |>
                   filter(time_id == 76),
               aes(x = x, y = y), 
...
maia_pred$x <- modis_full$x
maia_pred$y <- modis_full$y
maia_pred$estimate <- maia_pred$estimate[, 1]

maia_pred |>
    filter(time.id == 77) |>
    ggplot(aes(x = x, y = y, color = estimate)) +
    geom_tile(size = 4) +
    geom_point(data = modis_aqs_matched |>
                   filter(time_id == 77),
               aes(x = x, y = y), 
               color = "black",
               size = 3)
...
temp <- data.frame(est = results$ensemble.estimate[, 1],
           time_id = results$time.id,
           space_id = results$space.id,
           spacetime_id = results$spacetime.id,
           x = cmaq_full$x,
           y = cmaq_full$y)
